...
function main() {
    ...
    if (env.profiling) {
        alert("层数计入 " + profiler.format(countDuration), "调试信息");
    }
...
function createProgressBar() {
    ...
    try {
        ...
        alert("进度条资源已损坏！请重新下载包含所有文件的脚本。", "错误", true);
        return false;
    }
...
function showDialog() {
    ...
    try {
        ...
        alert("打开对话框时出错！请提出问题并尝试旧版本。", "错误", true);
        return false;
    }
...
function bootstrap() {
    ...
    try {
        ...
        if (env.version < 9) {
            alert("不支持 CS2 之前的 Photoshop 版本！", "错误", true);
            return "cancel";
        }
...
function loadResource(file) {
    var rsrcString;
    if (!file.exists) {
        alert("资源文件 '" + file.name + "' 因为缺少导出对话框！请下载此脚本附带的其余文件。", "错误", true);
        return false;
    }
    try {
        file.open("r");
        ...
        alert("读取资源文件失败 '" + file.name + "'!\n\n原因： " + error + "\n\n请检查它是否可供阅读并重新下载，以防它损坏。", "错误", true);
        return false;
    }
...
function makeMainDialog() {    
    ...
    radioAll.helpTip = "导出所有图层"; 
    radioAll.text = "所有图层"; 
    radioAll.value = true; 

    var radioSelected = grpExport.add("radiobutton", undefined, undefined, {name: "radioSelected"}); 
    radioSelected.helpTip = "仅导出选定的组。必须选择要启用的组。"; 
    radioSelected.text = "选定组"; 
    ...
    cbIgnorePrefix.helpTip = "忽略以以下开头的图层"; 
    cbIgnorePrefix.text = "忽略以以下开头的层 "; 

    var txtIgnorePrefix = grpIgnorePrefix.add('edittext {properties: {name: "txtIgnorePrefix"}}'); 
    txtIgnorePrefix.helpTip = "要匹配的前缀"; 
    ...
    var ddNameAs_array = ["使用图层名称（条带扩展）","使用图层名称（保留扩展名）","使用图层和父组名称","使用索引降序","使用索引升序"]; 
    ...
    var ddLetterCasing_array = ["保持","小写","大写"]; 
    ...
    txtPrefix.helpTip = "每个图层名称前都会添加前缀"; 
    ...
    txtSuffix.helpTip = "每个图层名称后都会添加后缀"; 
    ...
    btnCancel.helpTip = "关闭对话框并且不保存任何更改"; 
    btnCancel.text = "取消"; 
    ...
    cbTopGroupsAsFolders.helpTip = "组保存为目录。嵌套组中的图层将保存在其最顶层的组中。"; 
    cbTopGroupsAsFolders.text = "首个组作为文件夹"; 

    var cbTopGroupsAsLayers = grpGroupsAs.add("checkbox", undefined, undefined, {name: "cbTopGroupsAsLayers"}); 
    cbTopGroupsAsLayers.helpTip = "顶级组将在导出之前将其所有子级合并到一个图层中"; 
    ...
    cbTrim.helpTip = "导出前是否去除空格"; 
    cbTrim.text = "去除空格"; 

    var ddTrim_array = ["所有","使用trim()","组合"]; 
    ...
    cbPadding.helpTip = "导出前是否为每层添加内边距"; 
    cbPadding.text = "内边距"; 
    ...
    cbTifWithAlpha.text = "带 Alpha 通道"; 
    ...
    cbPdfWithAlpha.text = "带 Alpha 通道"; 
    ...
    cbTgaWithAlpha.text = "带 Alpha 通道"; 
    ...
    cbBmpWithAlpha.text = "带 Alpha 通道"; 
